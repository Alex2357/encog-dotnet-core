...
{
    public class ProductUrlBackPropagation2 : IExample
    {
        public String[] URLS =
        {
        ...
        public static ExampleInfo Info
        {
            get
            {
                var info = new ExampleInfo(
                    typeof(ProductUrlBackPropagation2),
                    "ProductUrlBackPropagation2",
                    "Product url testing",
...
//            var length = URLS.Length;
            ...
            for (var length=20;length < URLS.Length-1; length++)
            {
                ...
                var error = GetPredictionError(length, network);
                Console.WriteLine("Length={0}, error={1}", length, error);
                ...
                res =
                    network.Compute(
                        new StringMLData("http://www.dummydomain2.com/12v-li-pol-cordless-drill-skin-only_p6210356"))[0];
                res =
                    network.Compute(new StringMLData("http://www.dummydomain2.com/bla-bla-18v-hjsdfj-dhg_p3473985"))[0];
                res = network.Compute(new StringMLData("http://www.dummydomain2.com/our-range/hand-tools"))[0];
            }
        }

        private double GetPredictionError(int length, BasicNetwork network)
        {
            ...
            {
                var url = URLS[i];
                var actual = network.Compute(new StringMLData(url))[0];
                var actualComputed = actual > 0.5 ? 1 : 0;
                var expected = IsProductUrl(url) ? 1.0 : 0;
                var diff = actualComputed - expected;
                s += diff*diff;
                count++;
            }
            s /= count;
